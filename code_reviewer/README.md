...
### 4. Настройка контекстного меню Windows (ПКМ)

Чтобы добавить пункт **"Local Code Reviewer"** в контекстное меню проводника (ПКМ по файлу):

1. Запустите скрипт регистрации:
   ```bash
   python code_reviewer/register_context_menu.py
   ```
2. Теперь вы можете нажать правой кнопкой мыши на любой файл и выбрать **"Local Code Reviewer"**. Скрипт проанализирует весь файл.
